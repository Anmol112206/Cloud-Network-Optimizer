...
5. **Link Utilization (%)**: Determined by packets currently in transit (utilization is calculated based on the actual sizes of the packets in transit relative to link bandwidth):
   $$\text{Link Utilization} = \frac{\text{Active Packets in Transit Size (MB)}}{\text{Link Bandwidth (MBpt)}} \times 100$$
...
*   **Topology**: 3 Routers and 2 Links
    ...
    *   Link `L1` (`R1` $\leftrightarrow$ `R2`): Latency = 2 ticks, Bandwidth = 100 MBpt
    *   Link `L2` (`R2` $\leftrightarrow$ `R3`): Latency = 3 ticks, Bandwidth = 100 MBpt
...
*   **Expected Telemetry & Results**:
    ...
    *   **Link Utilization**: `L1` is 0.001% (2 active packets = 1000 Bytes = 0.001 MB in transit), `L2` is 0.0015% (3 active packets = 1500 Bytes = 0.0015 MB in transit).
...
*   **Topology**: 3 Routers and 2 Links
    ...
    *   Link `L1` (`R1` $\leftrightarrow$ `R2`): Latency = 1 tick, Bandwidth = 100 MBpt
    *   Link `L2` (`R2` $\leftrightarrow$ `R3`): Latency = 1 tick, Bandwidth = 100 MBpt
*   **Traffic Configuration**:
...
*   **Topology**: 4 Routers and 4 Links
    ...
    *   Link `L12` (`R1` $\leftrightarrow$ `R2`): Latency = 1 tick, Bandwidth = 100 MBpt
    *   Link `L24` (`R2` $\leftrightarrow$ `R4`): Latency = 6 ticks, Bandwidth = 100 MBpt
...
*   **Expected Telemetry & Results**:
    ...
    *   **Link Utilization**: `L13` is 0.001% (2 active packets), `L34` is 0.001% (2 active packets). `L12` and `L24` are 0% (no packets route through them).
...
*   **Topology**: 3 Routers and 2 Links
    ...
    *   Link `L1` (`R1` $\leftrightarrow$ `R2`): Latency = 1 tick, Bandwidth = 20 MBpt -- Low Bandwidth Link!
    *   Link `L2` (`R2` $\leftrightarrow$ `R3`): Latency = 1 tick, Bandwidth = 200 MBpt
*   **Traffic Configuration**:
    *   Stream: `R1` $\rightarrow$ `R3` (Rate = 3 packets/tick, packet size = 10,000,000 Bytes = 10 MB)
*   **Congestion & Bottlenecks**: **Bandwidth Bottleneck on Link `L1`**. The link carries 3 packets/tick. Since latency is 1, 3 active packets are on the link simultaneously. Each active packet has a size of 10 MB, representing 30 MB of virtual load in transit, requiring more than the 20 MBpt link bandwidth.
*   **Expected Telemetry & Results**:
    ...
    *   **Packet Loss**: 0%
    *   **Queues**: All queues remain at 0.
    *   **Link Utilization**: `L1` utilization is **150%** (30 MB usage / 20 MBpt bandwidth), turning the link red and dashed in the graph to show severe bandwidth congestion. `L2` utilization is 15% (30 MB usage / 200 MBpt bandwidth).
...
*   **Topology**: 4 Routers in Y-shape and 3 Links
    ...
    *   Link `L13` (`R1` $\leftrightarrow$ `R3`): Latency = 1 tick, Bandwidth = 100 MBpt
